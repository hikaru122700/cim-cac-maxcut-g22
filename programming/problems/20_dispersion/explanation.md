...
> **$D$ と $\beta_2$ の換算(具体例)**
> カタログでは波長基準の **分散パラメータ $D$**(単位 $\mathrm{ps/(nm\cdot km)}$)で書かれることが多い。$\beta_2$ との関係は
> $$ \beta_2 = -\frac{\lambda^2}{2\pi c}\,D $$
> である。標準 SMF の代表値 $D=17\ \mathrm{ps/(nm\cdot km)}$、$\lambda=1550\ \mathrm{nm}$、$c=2.998\times10^5\ \mathrm{nm/ps}$ を入れると
> $$ \beta_2 = -\frac{(1550)^2}{2\pi\times2.998\times10^5}\times17 \approx -21.7\ \mathrm{ps^2/km} $$
> となり、本問で使う $-22\ \mathrm{ps^2/km}$ とほぼ一致する(逆に $\beta_2=-22$ を $D$ に戻すと $D\approx17.2\ \mathrm{ps/(nm\cdot km)}$)。$D>0$ ($\beta_2<0$) が **異常分散** にあたる。
...
> **伝達関数の位相を数で見る**
> 2 次位相 $\phi(\omega)=\frac{\beta_2}{2}\omega^2 z$ に代表値を代入してみる。入力ガウスのスペクトル半値幅にあたる $\omega=1/T_0=0.166\ \mathrm{rad/ps}$($T_0=6.006$ ps、後述)、$\beta_2=-22\ \mathrm{ps^2/km}$ をとると、
>
> | $z$ [km] | $\phi=\frac{\beta_2}{2}\omega^2 z$ [rad] | $\phi/\pi$ |
> | ----: | ----: | ----: |
> | 1  | $-0.305$ | $-0.097$ |
> | 5  | $-1.525$ | $-0.485$ |
> | 10 | $-3.050$ | $-0.971$ |
>
> $\omega^2$ に比例するので、スペクトルの裾($\omega=2/T_0$)では位相が 4 倍($z=10$ km で $-12.2$ rad $\approx -3.88\pi$)まで膨れる。中心成分と裾成分でこれだけ位相がずれることが、時間波形が崩れて広がる直接の原因になる。
...
各距離で $z/L_D$ と広がり倍率 $\sqrt{1+(z/L_D)^2}$ を並べると次のようになる($L_D=1.639$ km)。

| $z$ [km] | $z/L_D$ | $\sqrt{1+(z/L_D)^2}$ | $T_\mathrm{FWHM}(z)$ [ps] |
| ----: | ----: | ----: | ----: |
| 0  | 0.000 | 1.000 | 10.00 |
| 1  | 0.610 | 1.171 | 11.71 |
| 2  | 1.220 | 1.577 | 15.77 |
| 5  | 3.050 | 3.210 | 32.10 |
| 10 | 6.100 | 6.181 | 61.81 |

$z=L_D=1.639$ km で倍率はちょうど $\sqrt2\approx1.414$ になる。$z=10$ km では $z/L_D=6.10$ なので、漸近形 $z/L_D$ による近似値 $61.0$ ps が厳密値 $61.81$ ps にすでに 1.3% まで迫っており、この距離で広がりがほぼ線形領域に入っていることが読み取れる。
