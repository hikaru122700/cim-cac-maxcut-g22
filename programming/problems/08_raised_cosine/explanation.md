...
> **帯域とサンプル数の数値例**
> $R_s = 50$ Gbaud(シンボル周期 $T = 1/R_s = 20$ ps)で $\alpha$ を変えると、占有帯域と必要サンプル数はこうなる。
>
> | $\alpha$ | $B=(1+\alpha)R_s$ | $f_s/R_s = B/R_s$ の最小値 | 最小 $f_s$ |
> | ---- | ---- | ---- | ---- |
> | $0$ | $50$ GHz | $1$ | $50$ Gsample/s |
> | $0.5$ | $75$ GHz | $1.5$ | $75$ Gsample/s |
> | $1$(本問) | $100$ GHz | $2$ | $100$ Gsample/s |
>
> $f_s/R_s$ は整数である必要があるので、$\alpha=1$ なら最小整数の $2$、つまり 2 sample/symbol を採る。
> 1 sample/symbol($f_s = 50$ Gsample/s)では $f_s = 50 < B = 100$ GHz でエイリアシングが起きる。
...
> **$h(t)$ を手で計算してみる($\alpha = 1$)**
> 本問の $\alpha = 1$ で、いくつかの $t/T$ について上式に代入する。第1因子 $\mathrm{sinc}(t/T)=\dfrac{\sin(\pi t/T)}{\pi t/T}$、
> 第2因子 $\dfrac{\cos(\pi\alpha t/T)}{1-(2\alpha t/T)^2}$ を別々に求めて掛ける。
>
> - $t=0$:第1因子は $\mathrm{sinc}(0)=1$、第2因子は $\cos 0 / (1-0)=1$。よって $h(0)=1\times 1=1$。**(自分のシンボル中心 = 1)**
> - $t=T/4$($t/T=0.25$):$\mathrm{sinc}(0.25)=0.9003$、第2因子 $=\dfrac{\cos(\pi/4)}{1-(0.5)^2}=\dfrac{0.7071}{0.75}=0.9428$。
>   よって $h=0.9003\times 0.9428=0.8489$。
> - $t=T/2$($t/T=0.5$):ここは分母 $1-(2\alpha t/T)^2 = 1-1^2 = 0$ で式が $0/0$ になる特異点。
>   極限をとると第2因子は $\pi/4=0.7854$ に収束する(`rc_filter` が $\pi/4$ で埋めている点)。
>   $\mathrm{sinc}(0.5)=0.6366$ を掛けて $h=0.6366\times 0.7854=0.5000$。
> - $t=T,\,2T$($m=1,2$):第1因子の $\mathrm{sinc}(m)=\dfrac{\sin(\pi m)}{\pi m}=0$ なので、第2因子が有限であれば
>   $h=0$。**(他のシンボル中心 = 0、これが Nyquist 条件)**
>
> まとめると次の表になる。$h(0)=1$、$h(T)=h(2T)=0$ が数値で確認でき、隣のシンボルへの干渉がゼロになっている。
>
> | $t/T$ | $\mathrm{sinc}(t/T)$ | 第2因子 | $h(t)$ |
> | ---- | ---- | ---- | ---- |
> | $0$ | $1$ | $1$ | $\mathbf{1}$ |
> | $0.25$ | $0.9003$ | $0.9428$ | $0.8489$ |
> | $0.5$ | $0.6366$ | $0.7854\;(=\pi/4)$ | $0.5000$ |
> | $1$ | $0$ | — | $\mathbf{0}$ |
> | $2$ | $0$ | — | $\mathbf{0}$ |
