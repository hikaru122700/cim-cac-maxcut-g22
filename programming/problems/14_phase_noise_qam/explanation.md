...
> **数値例: 1 つのシンボルを回してみる**
> 16QAM の角の手前のシンボル「$3+1j$」を考える。`comm.bits_to_symbols` は平均電力 1 に規格化する
> ため $\sqrt{2(M-1)/3}=\sqrt{10}$ で割るので、規格化後は
> $$ s = \frac{3+1j}{\sqrt{10}} = 0.9487 + 0.3162j,\qquad |s| = 1,\quad \angle s = 18.43^\circ. $$
> これに位相 $\phi=5^\circ$ を掛けると、$e^{j\phi} = \cos5^\circ + j\sin5^\circ = 0.9962 + 0.0872j$ なので
> $$ s\cdot e^{j\phi} = 0.9175 + 0.3977j,\qquad |s\cdot e^{j\phi}| = 1,\quad \angle = 23.43^\circ. $$
> **絶対値は 1 のまま、偏角だけが $18.43^\circ \to 23.43^\circ$ とちょうど $5^\circ$ 増えた**。
> 位相 $\phi$ を変えながら同じ計算をすると次のようになる(規格化後の値)。
>
> | $\phi$ | $e^{j\phi}$ | $s\cdot e^{j\phi}$ | $\lvert s\cdot e^{j\phi}\rvert$ | 偏角 |
> | ----: | ---- | ---- | ----: | ----: |
> | $0^\circ$ | $1.0000 + 0.0000j$ | $0.9487 + 0.3162j$ | $1.000$ | $18.43^\circ$ |
> | $1^\circ$ | $0.9998 + 0.0175j$ | $0.9430 + 0.3327j$ | $1.000$ | $19.43^\circ$ |
> | $5^\circ$ | $0.9962 + 0.0872j$ | $0.9175 + 0.3977j$ | $1.000$ | $23.43^\circ$ |
> | $10^\circ$ | $0.9848 + 0.1736j$ | $0.8794 + 0.4762j$ | $1.000$ | $28.43^\circ$ |
> | $20^\circ$ | $0.9397 + 0.3420j$ | $0.7833 + 0.6216j$ | $1.000$ | $38.43^\circ$ |
>
> 列を縦に追うと、I と Q は変わるが絶対値は $1.000$ で固定され、偏角だけが $\phi$ ぶん素直に進む。
> これが「点が原点を中心とする円周(円弧)に沿って動く」ことの正体である。
...
> **数値例: 内側と外側で弧長を比べる**
> 16QAM の最内点「$1+1j$」と最外(角)点「$3+3j$」を規格化(割る $\sqrt{10}$)すると半径は
> $$ \rho_\text{内} = \frac{|1+1j|}{\sqrt{10}} = \frac{\sqrt2}{\sqrt{10}} = 0.447,\qquad
>    \rho_\text{外} = \frac{|3+3j|}{\sqrt{10}} = \frac{3\sqrt2}{\sqrt{10}} = 1.342. $$
> 系列全体で角度が $\Delta\theta = 1.4$ rad ぶん広がるとすると、弧長 $\rho\,\Delta\theta$ は
> $$ \text{内側: } 0.447\times1.4 = 0.63,\qquad \text{外側: } 1.342\times1.4 = 1.88 $$
> となり、外側は内側の **3 倍**($\rho_\text{外}/\rho_\text{内}=3$ そのまま)長く伸びる。
> 16QAM の最近接点間隔は規格化後 $2/\sqrt{10}=0.632$ なので、外側の弧 $1.88$ は隣の点との間隔を
> 大きく超えており、外周の点が隣の格子へ容易にはみ出すことが数字でも分かる。
