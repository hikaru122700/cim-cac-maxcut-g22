...
### トレードオフを数字で追う(10 スパンの U字)

10 スパンを例に、$P_\mathrm{in}$ を 3 dB ずつ上げたときの 2 つの量を並べると、谷ができる理由がはっきりする。左半分(SNR)はパワーを上げるほど良くなり、右半分(累積非線形位相)はパワーに比例して悪化する。累積位相は $N$ スパン分を足して $\phi_\mathrm{NL,tot}=\gamma\, P_\mathrm{in}\, L_\mathrm{eff}\, N$、ASE 限界 SNR は $\mathrm{SNR}=P_\mathrm{in}/(N S_\mathrm{ASE} R_s)$ で求まる($N=10$、$L_\mathrm{eff}=14.46$ km、$S_\mathrm{ASE}=1.61\times10^{-16}$ W/Hz、$R_s=32$ GBaud)。

| $P_\mathrm{in}$ [dBm] | $P_\mathrm{in}$ [mW] | ASE 限界 SNR [dB] | 累積非線形位相 $\phi_\mathrm{NL,tot}$ [rad] | 支配要因 |
| ---: | ---: | ---: | ---: | --- |
| $-6$ | 0.25 | 6.9 | 0.076 | ASE(SNR 不足) |
| $-3$ | 0.50 | 9.9 | 0.152 | ASE |
| $0$ | 1.00 | 12.9 | 0.304 | 釣り合い(谷) |
| $+3$ | 2.00 | 15.9 | 0.606 | 非線形が増加 |
| $+6$ | 3.98 | 18.9 | 1.209 | 非線形ひずみ |

低パワー側($-6$ dBm)では SNR が 6.9 dB しかなく ASE 雑音で BER が高い。$+3$ dB ごとに SNR は 3 dB 改善するが、その代償に累積非線形位相も 2 倍ずつ増える。$0$ dBm 付近で両者が釣り合って BER が最小になり($\approx1.6\times10^{-4}$、実行結果の表と一致)、そこから上げると SNR が伸びても非線形位相が $0.3\to0.6\to1.2$ rad と急増して QPSK の判定境界(各象限 $\pm45^\circ\approx0.79$ rad)を侵食し、BER が再び上がる。これがコンステレーション図の「最適(中)→ 高 Pin で再び広がる(右)」に対応する。

> スパン数 $N$ を増やすと、同じ $P_\mathrm{in}$ でも SNR は $1/N$(分母の $N$)、累積非線形位相は $N$ 倍で効く。両方が $N$ で悪化するため、谷の位置(最適パワー)はほぼ動かず谷の深さ(最小 BER)だけが浅くなる。これが本問の「最適 Pin は約 0 dBm に収束、最小 BER は 10→20→50 スパンで悪化」の正体になる。
...
> **1 ステップを数字で追う(非線形ステップ)** 本問は `STEPS_PER_SPAN = 10` なので、100 km スパンを $\Delta z = 100/10 = 10$ km 刻みで進める。非線形ステップで掛かる位相は $\gamma |A|^2 \Delta z$ で、$|A|^2$ がその点の瞬時パワーになる。例として平均パワー $P_\mathrm{in} = 0\ \mathrm{dBm} = 1\ \mathrm{mW}$ の QPSK をスパン先頭($z=0$、損失前)で見ると、瞬時パワーがおよそ $P_\mathrm{in}$ のシンボル中心では
> $$ \phi_\mathrm{NL} = \gamma\, P_\mathrm{in}\, \Delta z = 2.1 \times 10^{-3} \times 10 = 0.021\ \mathrm{rad}\ (\approx 1.2^\circ) $$
> だけ位相が回る。$\Delta z$ が小さいので 1 ステップの回転はごくわずかで、これを 10 ステップ積み重ねる。ただし損失で強度が下がるため後段のステップほど $|A|^2$ が小さくなり、回転も弱まる。スパン全体での実効的な回転は、有効長 $L_\mathrm{eff}=14.46$ km を使った $\gamma P_\mathrm{in} L_\mathrm{eff} = 2.1\times10^{-3}\times14.46 = 0.030\ \mathrm{rad}$ で見積もれる($\Delta z$ 刻みの和が解析積分にほぼ一致する)。$\Delta z$ を半分にすればステップ数が倍になり位相の刻みは半分になるが、合計はこの値に収束する。これが分割幅 $\Delta z$ を十分小さく取る理由になる。
