...
$$ N_0 = \frac{P_\text{symbol}}{\mathrm{SNR_{lin}}} = \frac{1}{10^{20/10}} = \frac{1}{100} = 0.01 $$

とすればよい。コードでは `signal_power=1.0`(シンボル電力)を明示して `comm.add_awgn` を呼ぶ。

> **計算例: 雑音標準偏差 σ を求める**
> $N_0$ は複素雑音の総電力。`add_awgn` はこれを実部・虚部へ半分ずつ分配するので、1 軸あたりの分散は $N_0/2 = 0.005$、標準偏差は
> $$ \sigma = \sqrt{N_0/2} = \sqrt{0.005} \approx 0.0707 $$
> となる。実際に生成される雑音 1 サンプルは $n = \sigma(g_\mathrm{I} + j\,g_\mathrm{Q})$($g_\mathrm{I}, g_\mathrm{Q}\sim\mathcal{N}(0,1)$)で、その期待電力は $\mathbb{E}|n|^2 = \sigma^2 + \sigma^2 = 2\sigma^2 = 0.01 = N_0$ と、ちゃんと $N_0$ に戻る。

> **計算例: `signal_power` を省略すると何 dB ずれるか**
> 整形後の波形 `shaped` の実測平均電力は(4QAM, α=1, 2 sps の場合)
> $$ \langle|\text{shaped}|^2\rangle \approx 0.750 $$
> でシンボル電力 1 より小さい。0 挿入アップサンプルで電力 0 のサンプルが半分混ざるためである。`signal_power` を省略するとこの 0.750 を基準に $N_0 = 0.750/100 = 0.0075$ で雑音を決めてしまう。雑音はこのまま判定点にも乗るので、判定点 SNR は
> $$ 10\log_{10}\!\frac{1}{0.0075} \approx 21.25\ \text{dB} $$
> となり、狙いの 20 dB から約 1.25 dB もずれる。これが `signal_power=1.0` を明示する理由である。
...
> **数値トレース: 1 サンプルに雑音を足す前後(4QAM, SEED=3, シンボル 0)**
> 群遅延 `delay=12` なので、シンボル 0 の中心は `shaped[12]` にある。Nyquist 性によりここは元シンボルそのもの。そこへ 1 個ぶんの複素雑音が乗る。
>
> | 量 | 値 |
> | ---- | ---- |
> | 送信シンボル $s_0$ | $0.7071 + 0.7071j$($=1/\sqrt2 + j/\sqrt2$) |
> | 判定点信号 `shaped[12]` | $0.7071 + 0.7071j$(ISI なしで一致) |
> | 加える雑音 $n$ | $-0.0199 + 0.0449j$ |
> | 受信シンボル `rx[12]` $=s_0+n$ | $0.6872 + 0.7520j$ |
>
> この 1 サンプルの雑音電力は $|n|^2 = (-0.0199)^2 + (0.0449)^2 \approx 0.00241$ で、平均的には $N_0=0.01$ になる(1 個では揺らぐ)。判定点では信号 $\approx 0.7071$ がほぼそのまま残り、雑音だけが $\sigma\approx 0.0707$ 程度の小さなずれとして上乗せされる。$5\times10^5$ サンプル分を平均すると雑音電力が 0.01 に収束し、出力の実測 SNR が 20.00 dB になる。
