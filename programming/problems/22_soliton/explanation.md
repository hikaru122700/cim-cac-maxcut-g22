...
> 数値で見る $L_D=L_{NL}$
> 本問の値($T_0=5.673$ ps、$\beta_2=-22$ ps²/km、$P_0=0.3255$ W)で 2 つの長さを別々に計算してみる。
> $$ L_D=\frac{T_0^2}{|\beta_2|}=\frac{5.673^2}{22}=1.463\ \text{km},\qquad L_{NL}=\frac{1}{\gamma P_0}=\frac{1}{2.1\times0.3255}=1.463\ \text{km}. $$
> 別の式から出したのに同じ値になる。$N=1$ とは「分散が効き始める距離」と「非線形が効き始める距離」がぴったり一致する状態のことで、$N^2=L_D/L_{NL}=1$ はそれを一言で表している。どちらか一方だけ先に効くということがないので、釣り合いが保たれる。
...
入力パルス($z=0$)の値を数点だけ書き出すと次のようになる($P_0=0.3255$ W、$T_0=5.673$ ps)。

| $T$ [ps] | $T/T_0$ | 振幅 $\sqrt{P_0}/\cosh(T/T_0)$ [√W] | 強度 $P$ [W] | $P/P_0$ |
| ---: | ---: | ---: | ---: | ---: |
| 0 | 0.000 | 0.5705 | 0.3255 | 1.0000 |
| 2.5 | 0.441 | 0.5193 | 0.2697 | 0.8284 |
| 5.0 | 0.881 | 0.4034 | 0.1628 | 0.5000 |
| 10.0 | 1.763 | 0.1902 | 0.0362 | 0.1111 |
| 15.0 | 2.644 | 0.0807 | 0.0065 | 0.0200 |

中央 $T=0$ で強度がピーク $P_0=0.3255$ W。$T=\pm5$ ps でちょうど $P/P_0=0.5$(半分)になっており、その差 10 ps が指定どおりの FWHM になっている。$T_0=5.673$ ps は半値点($T=5$ ps)より少し外側で、$\cosh(1)\approx1.543$ から振幅がピークの $1/1.543=0.648$ 倍、強度がその 2 乗で $0.42$ 倍に落ちる点にあたる。sech は裾が指数関数的に減衰し、$T=15$ ps では強度がピークの 2% まで下がる。この同じ表が $z$ をいくら進めても変わらない、というのがソリトンである。
