...
> **数値で確認(SNR=15 dB の雑音電力)**
> シンボル電力を 1 に固定しているので、$\mathrm{SNR}=15\,\mathrm{dB}$ では雑音電力は
> $N_0 = 1 / 10^{15/10} = 10^{-1.5} \approx 0.0316$。複素雑音なので実部・虚部に半分ずつ分配され、
> 各軸の標準偏差は $\sigma = \sqrt{N_0/2} \approx 0.126$。整形をかけても、ISI-free 点で抜き出した
> サンプルは「元シンボル + この分散の複素ガウス雑音」になり、整形なしの 問2-1 と同じ値になる。
...
> **数値で確認(16QAM, SNR=15 dB の解析解)**
> 16QAM では $M=16$、$\log_2 M = 4$、$\sqrt M = 4$ なので前係数は
> $\frac{4}{4}\left(1-\frac14\right) = 0.75$。$\gamma_s = 10^{15/10} \approx 31.6$ なので
> 平方根の中は $\frac{3}{16-1}\times 31.6 = 0.2\times 31.6 \approx 6.32$、つまり $Q$ の引数は
> $\sqrt{6.32} \approx 2.51$。$Q(2.51) \approx 5.95\times10^{-3}$ より
> $$ \mathrm{BER} \approx 0.75 \times 5.95\times10^{-3} \approx 4.5\times10^{-3}. $$
> この後の表で見るとおり、整形ありの実測値もほぼ同じ $4.3\times10^{-3}$ に乗る。

代表点の解析解と整形あり実測値を並べると、対応の良さが数字で見える(`SEED=5`、30 万シンボル)。

| QAM | SNR [dB] | 解析解 BER | 整形あり実測 BER |
| --- | --- | --- | --- |
| 4QAM | 10 | $7.8\times10^{-4}$ | 同オーダー(問2-1 と一致) |
| 16QAM | 10 | $5.90\times10^{-2}$ | $5.90\times10^{-2}$ |
| 16QAM | 15 | $4.47\times10^{-3}$ | $4.33\times10^{-3}$ |
| 16QAM | 20 | $2.90\times10^{-6}$ | $8.3\times10^{-7}$(誤り数が少なく統計揺らぎが大きい) |
| 64QAM | 20 | $8.49\times10^{-3}$ | 同オーダー |
| 256QAM | 25 | $1.26\times10^{-2}$ | 同オーダー |

BER が $10^{-5}$ を切るあたり(例 16QAM, 20 dB)では 30 万シンボルでも誤りが数個しか出ず、
実測値が解析解からオーダーでずれて見えることがある。これは整形のせいではなく、後述する
**測定下限**(§5)による統計的な揺らぎで、サンプル数を増やせば解析解に近づく。
...
> **数値で確認(整形後はサンプル平均電力が 1 でない)**
> シンボル列 `tx` の平均電力は $\overline{|tx|^2}\approx 1.00$ だが、$\alpha=1$ / 2 sps で整形した
> サンプル列 `shaped` の平均電力を測ると $\overline{|\mathrm{shaped}|^2}\approx 0.75$ になる。
> アップサンプル時に挿入したゼロサンプルや、波形が時間方向に広がることで、サンプル基準の平均電力が
> シンボル電力からずれるためである。もし `signal_power` を省いてこの $0.75$ を基準に雑音を載せると、
> 雑音電力が $N_0 = 0.75/\gamma_s$ と本来の $1/\gamma_s$ より小さくなり、横軸の SNR が実効的に
> $10\log_{10}(1/0.75)\approx 1.25\,\mathrm{dB}$ ぶん良い方へずれて、曲線が解析解より下に寄ってしまう。
> `signal_power=1.0` を明示することで判定点での SNR の基準をシンボル電力に固定している。
