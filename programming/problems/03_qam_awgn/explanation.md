...
> **数値例: I/Q への分散の配分**
> 本問の $N_0 = 0.01$(求め方は次節)を I/Q に分けると、各軸の分散は
> $$ \mathrm{Var}(n_I) = \mathrm{Var}(n_Q) = \frac{N_0}{2} = \frac{0.01}{2} = 0.005 $$
> となり、複素雑音 1 サンプルの期待電力は両軸の和に戻って
> $$ E[|n|^2] = E[n_I^2] + E[n_Q^2] = 0.005 + 0.005 = 0.01 = N_0 $$
> と $N_0$ に一致する。「複素 1 個ぶんの電力 $N_0$ を実部・虚部の 2 軸へ等分する」のがポイントで、片軸だけ見ると分散は半分の $0.005$ になる。
...
> **数値例: dB → 線形 → $\sigma$ を一気に追う**
> SNR = 20 dB を出発点に、付加すべき雑音の大きさが決まるまでを順に計算すると次のようになる。
>
> | ステップ | 式 | 数値 |
> | ---- | ---- | ---- |
> | dB → 線形 SNR | $10^{20/10}$ | $10^2 = 100$ |
> | 雑音総電力 $N_0$ | $P_s / \mathrm{SNR_{lin}} = 1/100$ | $0.01$ |
> | 1 軸の分散 | $N_0/2 = 0.01/2$ | $0.005$ |
> | 1 軸の標準偏差 $\sigma$ | $\sqrt{0.005}$ | $\approx 0.0707$ |
>
> dB の「20」は指数では「2」になる($/10$ してから $10$ の肩に乗せる)点に注意。逆向きに $\sigma$ から SNR を復元すると $N_0 = 2\sigma^2 = 0.01$、$\mathrm{SNR_{lin}} = 1/0.01 = 100$、$10\log_{10}100 = 20$ dB と元に戻る。
...
> **数値例: 1 シンボルに雑音を足す前後**
> 4QAM の送信点 $s = (1 + j)/\sqrt{2} \approx 0.7071 + 0.7071\,j$(電力 $|s|^2 = 1$)を例にとる。標準正規乱数が $u = 0.5$(実部用)、$v = -1.2$(虚部用)と出たとすると、各軸の雑音は $\sigma$ 倍して
> $$ n_I = \sigma u = 0.0707 \times 0.5 = 0.0354,\qquad n_Q = \sigma v = 0.0707 \times (-1.2) = -0.0849. $$
> 受信点は単純に足し算で
> $$ r = s + (n_I + j\,n_Q) = (0.7071 + 0.0354) + j\,(0.7071 - 0.0849) \approx 0.7425 + 0.6223\,j. $$
> 送信点から I 軸へ $+0.035$、Q 軸へ $-0.085$ ずれた。判定境界は原点を通る軸($I=0$, $Q=0$)なので、両軸とも符号が変わらず正しく $(+,+)$ の象限にとどまる。この 1 サンプルの雑音電力は $|n|^2 = 0.0354^2 + 0.0849^2 \approx 0.0085$ で、$10^6$ サンプル平均すれば設計値 $N_0 = 0.01$ に収束し、出力の「実測SNR ≈ 20.00 dB」になる。
