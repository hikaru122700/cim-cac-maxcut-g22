...
> **数値例: $T/2$ ずれた点で 1 サンプルを抜くと何が混ざるか**
>
> 本問は $R_s=32$ GHz、$\mathrm{sps}=2$ なので $f_s=64$ GHz、$T=31.25$ ps、サンプル間隔は
> $T_s=15.625$ ps $=T/2$。サンプリング位相が中心から $+T/2$ ずれた 1 サンプルを抜くと、
> 目的シンボル $a_0$ から見て各シンボルは $t/T = k+0.5$ の位置にあり、α=0 の sinc 値で重み付けされて足し込まれる。
>
> | 隣接シンボル $k$ | $-3$ | $-2$ | $-1$ | $0$ | $+1$ | $+2$ | $+3$ |
> | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- |
> | 重み $\mathrm{sinc}(k+0.5)$ | $+0.127$ | $-0.212$ | $\mathbf{+0.637}$ | $\mathbf{+0.637}$ | $-0.212$ | $+0.127$ | $-0.091$ |
>
> 抜いた 1 サンプルは $y=\sum_k a_k\,\mathrm{sinc}(k+0.5)$。最も強い項でも振幅 $0.637$ しかなく、
> しかも $a_{-1}$ と $a_0$ が**同じ重み**で混ざる。最強項を「欲しい信号」、残りすべてを ISI とみなすと
> 信号対干渉比は **SIR $\approx -1.6$ dB**(干渉のほうが大きい)。1 個の複素係数を掛けても、
> この足し算された別シンボルを引き算できないので復調不能になる。これが BER $\approx 0.25$ の正体である。
...
> **数値例: タップ数 → カバーできる時間窓・遅延**
>
> タップは $T_s=T/2$ 間隔なので、$L$ タップの窓は時間スパン $(L-1)\,T_s=(L-1)\,T/2$ をカバーする。
> 中心タップを基準にすると、前後に $\pm(L-1)/4$ シンボルぶんの遅延・タイミングずれを吸収できる
> ($f_s=64$ GHz, $T=31.25$ ps)。
>
> | タップ数 $L$ | 窓スパン [$T_s$] | 窓スパン [$T$] | 窓スパン [ps] | 吸収できる遅延 [$T$] |
> | ---- | ---- | ---- | ---- | ---- |
> | 1 | 0 | 0.0 | 0.00 | $\pm 0.00$ |
> | 2 | 1 | 0.5 | 15.63 | $\pm 0.25$ |
> | 4 | 3 | 1.5 | 46.88 | $\pm 0.75$ |
> | 6 | 5 | 2.5 | 78.12 | $\pm 1.25$ |
> | 10 | 9 | 4.5 | 140.63 | $\pm 2.25$ |
> | 16 | 15 | 7.5 | 234.38 | $\pm 3.75$ |
>
> 本問のタイミングずれは $T/2$ なので、表より $L=2$ でも時間窓自体は届く。それでも $L=10$ を使うのは、
> α=0 の sinc 裾が $\pm 2$ シンボル以上に広がっており(上の重み表参照)、整合フィルタ利得を得るには
> **裾までカバーする窓($L=10$ で $\pm 2.25\,T$)** が要るからである。$L=1$($\pm 0\,T$)では裾を 1 個も拾えない。
