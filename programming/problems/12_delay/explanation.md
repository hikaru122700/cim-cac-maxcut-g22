...
> **数値例: 半サンプル遅延でサンプル値がどう変わるか**
>
> 一番見やすいのは、シンボル列が $s[0]=1$ の単一インパルスだけの場合(他は 0)。連続波形は
> $x(t)=h(t/T)$ そのもので、$T=20$ ps、$T_s=10$ ps、α=1。まず **遅延なし** の 2 sps サンプルは
> 引数 $t/T = m\cdot 0.5$ で読むので、ちょうどパルスの格子点に乗る。
>
> | $m$ | $t=mT_s$ | 引数 $t/T$ | $x_0[m]=h(t/T)$ |
> | --- | --- | --- | --- |
> | 0 | 0 ps | 0.00 | $+1.000000$ |
> | 1 | 10 ps | 0.50 | $+0.500000$ |
> | 2 | 20 ps | 1.00 | $\;\;0.000000$ |
> | 3 | 30 ps | 1.50 | $\;\;0.000000$ |
>
> 次に **τ=5 ps(= 0.5 サンプル)遅延**。同じサンプル時刻 $t=mT_s$ で $x(mT_s-\tau)=h\!\big((mT_s-5)/T\big)$
> を読むと、引数が $0.5$ 刻みの格子から $0.25$ ずれて格子の「間」を読みに行く。
>
> | $m$ | $t-\tau$ | 引数 $(t-\tau)/T$ | $x_\tau[m]=h(\cdot)$ |
> | --- | --- | --- | --- |
> | 0 | $-5$ ps | $-0.250$ | $+0.848826$ |
> | 1 | $+5$ ps | $+0.250$ | $+0.848826$ |
> | 2 | $+15$ ps | $+0.750$ | $+0.169765$ |
> | 3 | $+25$ ps | $+1.250$ | $-0.024252$ |
>
> 遅延なしでは $(1.0,\,0.5,\,0,\,0)$ という鋭いピークだったのが、半サンプル遅らせると
> $(0.849,\,0.849,\,0.170,\,-0.024)$ と **山が両隣に分配** され、ピーク位置がサンプル格子の中間
> (5 ps 付近)へ移ったことがわかる。これは単に隣のサンプルをコピーしたものではなく、
> パルス $h$ の連続形を中間時刻で読み直した値である(複数シンボルがあれば全シンボルの裾が
> その中間時刻で重なり合う)。整数サンプル遅延ならこの引数がちょうど $0.5$ 刻みに戻り、
> 値は格子点そのもの=単なる配列ずらしになる。

> **別解(周波数領域の遅延定理)**: FFT で位相回転 $X(f)\to X(f)e^{-j2\pi f\tau}$ を掛けて逆変換しても
> 同じ端数遅延が作れる。位相シフト $-2\pi f\tau$ を τ=5 ps で代表周波数に代入すると次のようになる。
>
> | $f$ | $2\pi f\tau$ | 位相シフト | $e^{-j2\pi f\tau}$ |
> | --- | --- | --- | --- |
> | 0 | 0 rad | $0^\circ$ | $1.000$ |
> | 12.5 GHz | 0.3927 rad | $22.5^\circ$ | $0.924-0.383j$ |
> | 25 GHz | 0.7854 rad | $45^\circ$ | $0.707-0.707j$ |
> | 50 GHz | 1.5708 rad | $90^\circ$ | $-1.000j$ |
>
> 低周波ほど位相回転が小さく、高周波ほど大きく回る。α=1 の整形帯域端 $(1+\alpha)R_s/2 = R_s = 50$ GHz
> でちょうど $90^\circ$ 回る計算になる。直流(f=0)は回らないので平均値は遅延で変わらない。
> ここではパルスの式を直接使う、より見通しのよい方法を採った。
