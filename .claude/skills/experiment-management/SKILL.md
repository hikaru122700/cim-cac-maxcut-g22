...
> ### ★ 鉄則:数式解説は必ず「画像の数式」に基づく
> RESULTS.md 内で数式を使って説明するときは、**必ず** `docs/assets/base_update_equations.png`
> の数式を出発点とし、**その記法・記号をそのまま使う**(独自の文字・別表記を導入しない)。
> 任意の式変形・新しい式は、この基準式から「何を置き換えた / 足した結果か」を示す形で書く。
> 画像にない記号を使う必要が出たら、必ず基準式の記号との対応を併記する。
...
| $b$, $\mathbf{J}$ | 結合係数 と 結合行列(相互作用) |
| $c$, $\mathbf{n}_0$ | ノイズ強度 と ノイズベクトル |
| $\alpha$ | 利得スケール |
| $p(n)$ | ポンプ(時間依存) |
| $\beta$ | 飽和係数(非線形項 $\beta|\mathbf{F}(n)|^2$) |

**この実験での変更:**

| 記号/項 | 基準式 | 変更後 | 変更理由 |
|---|---|---|---|
| $p(n)$(ポンプ) | 全レプリカ共通の固定/単一ランプ | $p_r(n)=\text{mult}_r\,p_{\rm ramp}(n)$ の各レプリカ独立ランプ | 各レプリカに焼きなまし(時間発展)を持たせるため |
| (該当記号) | ... | ... | ... |

> 変更がない記号は表に載せない。変更した式は、基準式の記法のまま変更後の形を $$...$$ で再掲する。
...
## 6. 考察 — 何が効いたのか
（なぜその結果になったかの物理的・アルゴリズム的理由。数式変更がどう効いたかに触れる）
...
> **基準式画像の参照パス**: 基準式は `docs/assets/base_update_equations.png` に固定保存
> 済み。RESULTS.md の場所は `results/<date>/<kind>/v{N}_<desc>/` なので、そこからの
> 相対パスは `../../../../docs/assets/base_update_equations.png`(4 階層上)になる。
> 階層が違う場合は実際の深さに合わせて `../` の数を調整する。

執筆のコツ:
- 解説・ラベルは**日本語**。
- **数式解説は必ず基準式画像(`docs/assets/base_update_equations.png`)の記法に基づく**
  (ステップ3「★鉄則」参照)。論文の式番号(`Eq.(3)`)はその上で補助的に併記してよい。
