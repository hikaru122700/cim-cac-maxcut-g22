...
MAX-CUT 問題に対して、**6 手法すべてをデータセットごとに Optuna で調整**したソルバ
（物理インスパイア型 **CIM**・**CAC**、古典ヒューリスティクス **SA**・**SB**・**PT-ICM**・
本研究で新規実装した メメティック **GA**）を、**同一の実時間軸・統一の重み付きカット採点**で比較した。
...
1. **単体**では、調整済みの古典ヒューリスティクス **GA・SB** が最良格で、**単一手法は全データセットを
   支配しない**（G22・K2000 は GA、G55・G70 は SB）。SB が平均 gap 最小で最安定。CIM は高速だが
   頭打ちし、密 K2000 では再調整しても古典に届かない。**「ノーマル SA が弱い」のは調整不足が主因で、
   調整後は中位**（K2000 gap 1348→756）。
2. **ハイブリッドの価値は「救済」**: CIM/CAC で作った初期配置から TS/SA を走らせると、同じ精錬予算の
   乱数初期（cold-start）を大きく上回り、**物理ソルバの頭打ちを突破して競争圏へ引き上げる**
   （K2000 で CIM gap 794→120）。引き渡す盆地の質が結果を支配する。**ただし十分に調整された
   最良単体（GA/SB）は上回らない**——物理ソルバは「探索器・ポートフォリオの一員」として価値がある。
