...
def simulate_ga_batch(
    ...
    *,
    ...
    population_observer: Optional[Callable] = None,
):
    ...
    Args:
        ...
        init_population : (num_trials, pop_size, n) の全初期集団(±1 or 0/1)。
                          init_signs と併用不可。通常と同じ初期 TS を適用する。
        population_observer : callback(generation, populations, fitnesses)。
                          -1 は初期 TS 前、0 は初期 TS 後、1 以降は集団更新後。
                          配列はコピーを渡す。観測によって乱数列は変わらない。
    ...
    """
    if init_population is not None and init_signs is not None:
        raise ValueError("init_population and init_signs are mutually exclusive")
    full_init = None
    if init_population is not None:
        full_init = np.asarray(init_population)
        if full_init.shape != (num_trials, pop_size, n):
            raise ValueError("init_population must have shape (num_trials, pop_size, n)")
        if not np.all(np.isin(full_init, [-1, 0, 1])):
            raise ValueError("init_population must contain only binary spins")
        full_init = (full_init > 0).astype(np.int8)
    ...
    # 全初期集団を渡す場合も通常と同じ初期乱数を消費し、交叉の乱数を対応させる。
    if full_init is not None:
        init_batch[:] = full_init.reshape(num_trials * pop_size, n)
    if population_observer is not None:
        initial_fits = _cut_batch(n, indptr, indices, data, init_batch)
        population_observer(-1, init_batch.reshape(num_trials, pop_size, n).copy(),
                            initial_fits.reshape(num_trials, pop_size).copy())
    ...
    if population_observer is not None:
        population_observer(0, pops.copy(), fits.copy())
    ...
    while gen < max_generations:
        ...
        gen += 1
        if population_observer is not None:
            population_observer(gen, pops.copy(), fits.copy())
        if return_history:
