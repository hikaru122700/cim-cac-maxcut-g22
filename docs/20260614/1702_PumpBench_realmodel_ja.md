...
![本物モデルでの平均(棒=平均±標準偏差)と最良(星)。G22, 100 trial。**平均では全条件が拮抗するが、最良(星)では CAC の 13358 のみが既知ベスト線(赤破線)に到達する。**棒グラフ(平均)だけを見ると CAC の最良到達は見えない点に注意。](realmodel_comparison_best.png)
