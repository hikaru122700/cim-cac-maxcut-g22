...
> **注意（実測に基づく）**: 「捏造防止（Anti-Fabrication）」が保証するのは *引用と数値の出所が実在すること* であって、*実験モデルが現実を忠実に再現していること* ではない。サンドボックス実験は計算量制約から簡略化モデル（小さい `T`/`N`、近似ダイナミクス）になりがちで、**絶対性能や手法ランキングは本物の実装での再検証が必須**。実例は §13。
...
## 13. 実運用での検証所見 — PumpBench を G22 で本物モデル再検証（2026-06-14）

本資料の §1–§12 は README ベースの機能紹介だが、本節は **実際にこのツールで CIM 研究テーマを 1 本走らせ**（テーマ「CIM のポンプ電力関数の最適化」）、生成された論文 **PumpBench** の実験を**本物の実装で再検証**して得た実運用所見である。これがツールの実力と限界を最も正直に表す。

### 13.1 何が生成されたか

- 論文一式 `PumpBench: Learned CIM Pump Schedules Fall Short of a Tuned Sigmoid`（本文・LaTeX・`references.bib`・図・実験コード `code/`）。
- 中心主張: 「**開ループのポンプ形状は小レバー、閉ループフィードバック（CAC）が大レバー**」。8 条件を計算量整合で比較。
- 出力先: `AutoResearchClaw/artifacts/pump_power_opt/deliverables/`。

### 13.2 落とし穴 — 実験は「おもちゃモデル」だった

- 生成実験は正規形の平均場 CIM（`dx=(-1+p-x²)x - ξ·Jx`、`x³` 飽和、**無次元**）を **T=100 ステップ・K=20 軌道**で積分する簡略版だった。
- 実 G-Set 読込機能を持ちながら、**CPU 予算の都合で G22（N=2000）をスキップ**していた（`data.load_heldout` の `n<=1000` ガード）。
- そのまま G22 で評価すると絶対カット値が **78–96%** と低く、本リポジトリの検証済み実装（CIM 99.75% / CAC 99.99%）と比較できない。

### 13.3 本物モデルでの再検証（G22, 100 trial）

開ループ＝`modules/CIM.py`（Inoue–Yoshida ファイバーループ進行波）、閉ループ＝`modules/CAC.py`（Leleu 2021）で、同じ条件比較をやり直した。データ: `results/2026-06-14/pumpbench_real_cim_g22/`。

| 条件 | 平均 | 最良 | %既知ベスト |
|---|---|---|---|
| 線形利得ランプ（最良開ループ, g0∝k） | 13280.0 | 13337 | 99.84% |
| **CAC（本物・閉ループ）** | 13278.8 | **13358** | **99.99%** |
| 線形電力ランプ（現行ベースライン） | 13275.3 | 13326 | 99.75% |
| シグモイド | 13253.7 | 13313 | 99.66% |
| べき乗 早上げ p=0.5 | 13246.9 | 13306 | 99.60% |

- **全条件 99.6–99.99%** で過去実測と一致 — おもちゃモデルの歪み（78–96%）は消えた。**精度低下（リグレッション）は無い。**
- **論文の「シグモイド ≫ 線形」は本物模型で非再現** — むしろシグモイドは線形より悪い（指数飽和のファイバー模型では利得の早い立ち上げが不利。リポジトリのポンプ最適化実験 2026-06-08 の `シグモイド z=−22` とも一致）。**具体的なポンプ形状ランキングは転移しない。**
- 一方、**メタ結論（閉ループ CAC が本丸 / 開ループ形状は小レバー）は本物 G22 でも成立** — CAC のみ BKS−1（13358）に到達。ただし本物模型では両レバーとも小さい（開ループ形状差 ~0.7%）。

### 13.4 教訓 — バランスの取れた評価

- **強み**: テーマ分解・文献収集・仮説生成・実験コード骨格・執筆・査読・LaTeX まで一気通貫で「研究の足場」を高速に作る。捏造引用の自動排除も実働した。
- **限界**: サンドボックス実験は計算量制約から**簡略化モデル**（小さい `T`/`N`、近似ダイナミクス）になりがち。**生成された定量結論・絶対値・手法ランキングは、本物の実装・実インスタンスで再検証するまで信用しない**。「どのモデルで測ったか」を必ず確認する。
- **使い方の指針**: ARC は**仮説生成・構造化・初稿**に使い、**数値結論は検証済み実装で再実行して確定する**。本件（PumpBench → 本物 G22 再検証）がその実例。詳細考察: `results/2026-06-14/pumpbench_real_cim_g22/RESULTS.pdf`。

---

*出典: AutoResearchClaw README（aiming-lab/AutoResearchClaw, MIT License）。本資料は同 README をもとに日本語で要約・再構成し、§13 に本リポジトリでの実運用検証（2026-06-14）を追記したものである。*
