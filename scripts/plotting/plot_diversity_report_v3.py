...
def main():
    ...
    for k, ds in enumerate(datasets):
        ...
        style(ax)
    fig.suptitle("図1  手法の位置づけ：解は良いか／解はばらけるか", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    fig.savefig(run_dir / f"fig1_quadrant{sfx}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    for k, ds in enumerate(datasets):
        ...
        style(ax)
    fig.suptitle("図2  同じ実時間で 32 回解いたときの「散らばり」と「良さ」", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.985])
    fig.savefig(run_dir / f"fig2_overview{sfx}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    for k, ds in enumerate(datasets):
        ...
        style(ax)
    fig.suptitle("図4  共通の Tabu Search で磨いても、解の散らばりはほぼ変わらない", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    fig.savefig(run_dir / f"fig4_refine{sfx}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    fig.suptitle("図5  良い解は 1 か所に集まるのか、あちこちに散るのか（枠は各手法の重心）",
                 fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    fig.savefig(run_dir / f"fig5_bigvalley{sfx}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    fig.suptitle("図3  どれだけの頂点が「毎回同じ側」に決まるか（曲線が高いほど骨格が太い）",
                 fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    fig.savefig(run_dir / f"fig3_backbone{sfx}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title("図6  良い解を出しているのは誰か（全手法プールの上位 25%）", fontsize=13)
    style(ax)
    fig.tight_layout()
    fig.savefig(run_dir / f"fig6_topshare{sfx}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    fig.suptitle("図7  どの手法どうしが同じ場所を見ているか", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    fig.savefig(run_dir / f"fig7_cross{sfx}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    fig.suptitle("図8  解空間のどこに着地したか（近い点ほど似た解）", fontsize=14)
    fig.tight_layout(rect=[0, 0.07, 1, 0.94])
    fig.savefig(run_dir / f"fig8_mds{sfx}.png", bbox_inches="tight")
    plt.close(fig)
