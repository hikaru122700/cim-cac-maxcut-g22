"""plot_diversity_report_v2.py — 多様性評価の要約図（視認性強化版）。

v1 (`plot_diversity_report.py`) は色だけで 6 手法を区別していたため、
印刷・グレースケール・ざっと見のときに判別しづらい。v2 では

  - **手法ごとにマーカー形状・線種・ハッチを固定**（色に依存しない識別）
  - 凡例に頼らず**線や棒に直接ラベル**を置く
  - 注目手法（CIM）を太線・縁取りで強調
  - 4 象限マップで「良い/悪い × 均質/多様」の位置づけを一目で示す

を行う。出力は `fig*_v2.png`（v1 の図は上書きしない）。

使い方:
    python scripts/plotting/plot_diversity_report_v2.py <run_dir> [--suffix _v2]
"""
...
def legend_strip(fig, algos, y=0.005):
    ...
    fig.legend(handles=handles, loc="lower center", ncol=len(algos), fontsize=10,
...
def main():
    ...
    # ============================================================
    # fig0: 4 象限マップ（良い/悪い × 均質/多様）
    # ============================================================
    fig, axes = plt.subplots(2, 2, figsize=(11.6, 9.4), dpi=140, squeeze=False)
    for k, ds in enumerate(datasets):
        ...
        gsplit = float(np.median(gx))
        dsplit = float(np.median(dy))
        ax.axvline(gsplit, color="gray", ls="--", lw=1.0)
        ax.axhline(dsplit, color="gray", ls="--", lw=1.0)
        ax.set_xscale("log")
        lo, hi = gx.min() / 2.2, gx.max() * 2.6
        ax.set_xlim(lo, hi)
        pad = max(0.025, (dy.max() - dy.min()) * 0.36)
        ax.set_ylim(max(0.0, dy.min() - pad), min(0.52, dy.max() + pad))
        # 象限ラベル
        for xx, yy, txt, ha, va in [
            ...
            ax.text(xx, yy, txt, fontsize=9, color="#888", ha=ha, va=va)
        # ラベルが重ならないよう、x 順に上下交互へ振る
        by_x = sorted(algos, key=lambda a: R[ds][a]["gap_pct_mean"])
        side = {a: (1 if i % 2 == 0 else -1) for i, a in enumerate(by_x)}
        for a in algos:
            x, y = R[ds][a]["gap_pct_mean"], R[ds][a]["mean_pairwise"]
            big = a == HILITE
            ax.scatter([x], [y], s=420 if big else 200, marker=MARK[a][0],
                       color=ALGOS[a]["color"], edgecolors="k",
                       lw=2.0 if big else 0.8, zorder=3)
            off = (18 if big else 14) * side[a] - (8 if side[a] < 0 else 0)
            ax.annotate(ALGOS[a]["label"], (x, y), textcoords="offset points",
                        xytext=(0, off), ha="center",
                        va="bottom" if side[a] > 0 else "top",
                        fontsize=10 if big else 9,
                        fontweight="bold" if big else "normal",
                        bbox=dict(fc="white", ec="none", alpha=0.75,
                                  boxstyle="round,pad=0.12"))
        ax.set_xlabel("平均 gap [%]（左ほど高品質・対数軸）")
        ax.set_ylabel("平均ペアワイズ距離（上ほど多様）")
        ax.set_title(f"{ds}", fontsize=12)
        style(ax)
    fig.suptitle("手法の位置づけ：解は良いか／解はばらけるか", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    fig.savefig(run_dir / f"fig0_quadrant{sfx}.png", bbox_inches="tight")
    plt.close(fig)

    # ============================================================
    # fig1: 俯瞰（棒 + ハッチ + マーカー）
    # ============================================================
    fig, axes = plt.subplots(len(datasets), 2, figsize=(11.8, 3.1 * len(datasets)),
    ...
    for k, ds in enumerate(datasets):
        ...
        for yy, a in zip(y, srt):
            ...
            ax.text(v + 0.018, yy, f"{v:.3f}", va="center", fontsize=9,
                    fontweight="bold" if a == HILITE else "normal")
        ax.set_yticks(y)
        ax.set_yticklabels([ALGOS[a]["label"] for a in srt], fontsize=9.5)
        ...
        ax.set_title(f"{ds}: 出力解の散らばり", fontsize=11.5)
        ax.text(0.5, 0.04, " 無相関の水準", color="gray", fontsize=8.5,
        ...
        for yy, a in zip(y, srt):
            ...
            ax.barh(yy, v, color=ALGOS[a]["color"], alpha=0.4, edgecolor="k",
                    lw=0.6, hatch=MARK[a][2])
            ax.text(v + gap.max() * 0.03, yy, f"{v:.2f}", va="center", fontsize=9,
                    fontweight="bold" if a == HILITE else "normal")
        ax.set_yticks(y); ax.set_yticklabels([])
        ax.set_xlim(0, gap.max() * 1.3)
        ax.set_xlabel("平均 gap [%]（左ほど高品質）")
        ax.set_title(f"{ds}: 解の品質", fontsize=11.5)
        style(ax)
    fig.suptitle("同じ実時間で 32 回解いたときの「散らばり」と「良さ」", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.985])
    fig.savefig(run_dir / f"fig1_overview{sfx}.png", bbox_inches="tight")
    plt.close(fig)

    # ============================================================
    # fig2: 磨く前後（直接ラベル）
    # ============================================================
    fig, axes = plt.subplots(1, len(datasets), figsize=(3.9 * len(datasets), 5.0),
                             dpi=140, squeeze=False)
    for k, ds in enumerate(datasets):
        ax = axes[0][k]
        algos = [a for a in ORDER if a in RF[ds]]
        ys = np.array([RF[ds][a]["mean_pairwise"] for a in algos])
        lab_y = spread(ys, 0.022)
        for a, ly in zip(algos, lab_y):
            ...
            ax.plot([0, 1], [b, af], ls=MARK[a][1], marker=MARK[a][0],
                    color=ALGOS[a]["color"], lw=lw_of(a), ms=ms_of(a),
                    markeredgecolor="k", markeredgewidth=0.5)
            ax.annotate(ALGOS[a]["label"], (1.06, ly), fontsize=9,
                        color=ALGOS[a]["color"], va="center",
                        fontweight="bold" if a == HILITE else "normal")
        ax.set_xticks([0, 1]); ax.set_xticklabels(["磨く前", "磨いた後"], fontsize=10)
        ax.set_xlim(-0.2, 1.95)
        ax.set_ylim(0, 0.55)
        ax.set_ylabel("平均ペアワイズ距離" if k == 0 else "")
        ax.set_title(ds, fontsize=12)
        style(ax)
    fig.suptitle("共通の Tabu Search で磨いても、解の散らばりはほぼ変わらない", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    fig.savefig(run_dir / f"fig2_refine{sfx}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    fig, axes = plt.subplots(1, len(pair), figsize=(6.2 * len(pair), 5.4), dpi=140,
                             squeeze=False)
    for k, ds in enumerate(pair):
        ...
        for i, a in enumerate(algos):
            sel = owner == i
            big = a == HILITE
            ax.scatter(dref[sel], gaps[sel], s=110 if big else 55,
        ...
        ys = spread([min(c[2], ytop * 0.93) for c in cents], ytop * 0.085)
        for (a, cx, cy), ly in zip(cents, ys):
            ax.annotate(ALGOS[a]["label"], xy=(cx, min(cy, ytop * 0.97)),
                        xytext=(cx, ly), fontsize=9.5, color="k",
        ...
        if n_out:
            ax.text(0.02, 0.97, f"※ 軸外に {n_out} 点（最大 {gaps.max():.1f}%）",
                    transform=ax.transAxes, fontsize=9, color="#a33", va="top")
        kind = "疎グラフ" if ds != "K2000" else "密グラフ"
        note = "良い解は 1 か所に集中" if fdc > 0.6 else "良い解が広く散在"
        ax.set_title(f"{ds}（{kind}）  相関 = {fdc:.2f} → {note}", fontsize=12)
        style(ax)
    fig.suptitle("良い解は 1 か所に集まるのか、あちこちに散るのか（枠は各手法の重心）",
                 fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    fig.savefig(run_dir / f"fig3_bigvalley{sfx}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    fig, axes = plt.subplots(1, len(show), figsize=(6.4 * len(show), 5.2), dpi=140,
                             squeeze=False)
    for k, ds in enumerate(show):
        ...
        for a in algos:
            ...
            ax.plot(xs, m, ls=MARK[a][1], lw=lw_of(a), color=ALGOS[a]["color"],
                    solid_capstyle="round")
        # 50% 地点に直接ラベル
        vals = np.array([curves[a][curves[a].size // 2] for a in algos])
        lab_y = spread(vals, 0.055)
        for a, ly in zip(algos, lab_y):
            ax.annotate(ALGOS[a]["label"], (52, ly), fontsize=9.5,
        ...
        ax.text(99, 0.915, "「毎回同じ側」とみなす水準 ", color="gray", fontsize=9,
                ha="right")
        ax.set_xlabel("頂点（安定な順に並べた百分位）[%]")
        ax.set_ylabel("32 試行での偏り $|m_i|$" if k == 0 else "")
        ax.set_ylim(0, 1.04); ax.set_xlim(0, 100)
        ax.set_title(ds, fontsize=12)
        style(ax)
    fig.suptitle("どれだけの頂点が「毎回同じ側」に決まるか（曲線が高いほど骨格が太い）",
                 fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    fig.savefig(run_dir / f"fig4_backbone{sfx}.png", bbox_inches="tight")
    plt.close(fig)

    # ============================================================
    # fig5: 上位 25% の占有（ハッチ + 0 本の明示）
    # ============================================================
    fig, ax = plt.subplots(figsize=(9.6, 5.0), dpi=140)
    ...
    for x, ds in zip(xs, datasets):
        ax.text(x, bottom[x] + 1.5, "0 本: " + " / ".join(zero_algos[ds]),
                ha="center", fontsize=8.5, color="#a33")
    ax.set_xticks(xs); ax.set_xticklabels(datasets, fontsize=11)
    ax.set_ylim(0, bottom.max() + 9)
    ax.set_ylabel("上位 25% に入った解の本数（192 本中 約 48 本）")
    ax.set_title("良い解を出しているのは誰か（全手法プールの上位 25%）", fontsize=13)
    style(ax)
    fig.tight_layout()
    fig.savefig(run_dir / f"fig5_topshare{sfx}.png", bbox_inches="tight")
    plt.close(fig)
    ...
    fig, axes = plt.subplots(1, len(hm), figsize=(6.2 * len(hm), 5.4), dpi=140,
                             squeeze=False)
    for k, ds in enumerate(hm):
        ...
        for i in range(len(names)):
            for j in range(len(names)):
                ax.text(j, i, f"{M[i, j]:.2f}", ha="center", va="center",
                        fontsize=9.5, color="w" if M[i, j] > 0.38 else "k",
        ...
        for lb in list(ax.get_xticklabels()) + list(ax.get_yticklabels()):
            if lb.get_text() == ALGOS[HILITE]["label"]:
                lb.set_fontweight("bold")
        ax.set_title(f"{ds}: 手法間の解の隔たり（対角=手法内）", fontsize=12)
        fig.colorbar(im, ax=ax, label="平均距離（濃いほど遠い）", shrink=0.85)
    fig.suptitle("どの手法どうしが同じ場所を見ているか", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    fig.savefig(run_dir / f"fig6_cross{sfx}.png", bbox_inches="tight")
    plt.close(fig)

    # ============================================================
    # fig7: MDS（マーカー形状つき・最良解を注記）
    # ============================================================
    fig, axes = plt.subplots(1, len(hm), figsize=(6.2 * len(hm), 5.6), dpi=140,
                             squeeze=False)
    for k, ds in enumerate(hm):
        ax = axes[0][k]
        algos = [a for a in ORDER if a in R[ds]]
        ...
        for i, a in enumerate(algos):
            sel = owner == i
            sz = 40 + 200 * (1.0 - (gaps[sel] - lo) / max(hi - lo, 1e-9))
        ...
        ax.annotate("最良解", (emb[b, 0], emb[b, 1]), textcoords="offset points",
                    xytext=(22, 22), fontsize=10,
                    arrowprops=dict(arrowstyle="->", lw=1.4))
        ax.set_xlabel("MDS 第 1 軸"); ax.set_ylabel("MDS 第 2 軸" if k == 0 else "")
        ax.set_title(f"{ds}（点が大きいほど良い解）", fontsize=12)
        style(ax)
    legend_strip(fig, [a for a in ORDER if a in R[hm[0]]], y=0.0)
    fig.suptitle("解空間のどこに着地したか（近い点ほど似た解）", fontsize=14)
    fig.tight_layout(rect=[0, 0.07, 1, 0.94])
    fig.savefig(run_dir / f"fig7_mds{sfx}.png", bbox_inches="tight")
    plt.close(fig)

    print(f"fig0..fig7{sfx} -> {run_dir}")
